...
@Component({
  ...
  template: `
    <nav class="fixed top-0 z-50 w-full transition-all duration-300 bg-slate-900/95 backdrop-blur-md border-b border-white/5 shadow-2xl">
      ...
      @if (isMenuOpen()) {
        <div class="md:hidden bg-slate-900 border-t border-white/10 absolute w-full shadow-2xl z-40 h-screen animate-fade-in-down">
          <div class="px-6 py-8 space-y-6 flex flex-col items-center text-center">
            <a (click)="closeMenu()" routerLink="/" class="text-lg font-serif text-slate-300 hover:text-amber-500">Ana Sayfa</a>
            <a (click)="closeMenu()" routerLink="/fleet" class="text-lg font-serif text-slate-300 hover:text-amber-500">Kiralık Araçlar</a>
            <a (click)="closeMenu()" routerLink="/sales" class="text-lg font-serif text-amber-500 font-bold">2. El Galeri</a>
            <a (click)="closeMenu()" routerLink="/about" class="text-lg font-serif text-slate-300 hover:text-amber-500">Kurumsal</a>
            <a (click)="closeMenu()" routerLink="/contact" class="text-lg font-serif text-slate-300 hover:text-amber-500">İletişim</a>
          </div>
        </div>
      }
    </nav>

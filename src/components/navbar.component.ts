      ...
      <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-20 md:h-24">
          ...
          <div class="hidden lg:flex items-center space-x-8 lg:space-x-12">
            <a routerLink="/" routerLinkActive="text-amber-500 border-b-2 border-amber-500" [routerLinkActiveOptions]="{exact: true}" class="text-slate-300 hover:text-white font-medium text-xs uppercase tracking-widest transition-all duration-300 py-2">{{ t().nav.home }}</a>
            <a routerLink="/fleet" routerLinkActive="text-amber-500 border-b-2 border-amber-500" class="text-slate-300 hover:text-white font-medium text-xs uppercase tracking-widest transition-all duration-300 py-2">{{ t().nav.fleet }}</a>
          ...
          <div class="hidden lg:flex items-center space-x-4">
             ...
             <!-- Language Switcher -->
             <button (click)="toggleLang()" class="flex items-center text-xs font-bold text-slate-400 hover:text-white border border-slate-700 px-2 py-1 rounded transition-colors">
          ...
          <div class="lg:hidden flex items-center gap-4">
            ...
            <button (click)="toggleLang()" class="flex items-center text-xs font-bold text-slate-400 hover:text-white border border-slate-700 px-2 py-1 rounded">
                {{ uiService.currentLang() }}
            </button>
      ...
      @if (isMenuOpen()) {
        <div class="lg:hidden bg-slate-900/98 backdrop-blur-xl border-t border-white/10 absolute w-full shadow-2xl z-40 h-[calc(100vh-80px)] animate-fade-in-down overflow-y-auto">
          <div class="px-6 py-8 space-y-6 flex flex-col items-center text-center">
            <a (click)="closeMenu()" routerLink="/" class="text-2xl font-serif text-slate-300 hover:text-amber-500 py-2 w-full border-b border-white/5">{{ t().nav.home }}</a>
            <a (click)="closeMenu()" routerLink="/fleet" class="text-2xl font-serif text-slate-300 hover:text-amber-500 py-2 w-full border-b border-white/5">{{ t().nav.fleet }}</a>
            <a (click)="closeMenu()" routerLink="/sales" class="text-2xl font-serif text-amber-500 font-bold py-2 w-full border-b border-white/5">{{ t().nav.sales }}</a>
